...
from .session import AcpSessionManager, PermissionHandler, SessionConfig
...
class ACPLLMProvider:
    ...
    async def call_conversational(
        self,
        messages: list[dict[str, str]],
        conversation_handler: Callable[[str], str | None],
        max_turns: int = 10,
        **_kwargs: Any,
    ) -> str:
        """Multi-turn conversation with an ACP agent.

        After each response, calls ``conversation_handler(response_text)``.
        If it returns a string, that string is sent as the next prompt.
        If it returns None, the conversation ends and the last response is returned.

        Args:
            messages: Initial messages to send to the agent.
            conversation_handler: Callable that receives each response and returns
                a follow-up message or None to end.
            max_turns: Maximum number of follow-up turns (default 10).
        """
        pool = self._runtime.session_manager.get_pool(self._routing_key, self._cfg)
        prompt = self._to_acp_prompt(messages, None)
        ...
        async with pool.acquire(artifact_key=call_key) as session:
            ...
            for turn in range(max_turns):
                followup = conversation_handler(current)
                if followup is None:
                    return current
                logger.debug("Conversation turn %d: sending follow-up (%d chars)", turn + 1, len(followup))
                results = await session.prompt([text_block(followup)])
                current = "\n".join(results).strip()

            logger.warning("Conversation reached max_turns=%d, returning last response", max_turns)
            return current
